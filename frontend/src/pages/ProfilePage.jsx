...
const ProfilePage = ({ isOpen, onClose, user }) => {
  ...
  const handleVerifyUser = () => {
    alert(`${user?.email}로 인증번호가 발송되었습니다. (데모: 123456)`);
    setStep(2);
  };

  const handleVerifyCode = () => {
    if (formData.authCode === '123456') setStep(3);
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await axios.patch(`/users/${user.id}`, { password: formData.newPassword });
      setStep(4);
    } catch (error) {
      alert("변경 실패. 서버를 확인하세요.");
    }
  };
            ...
            {step === 2 && (
              <div className="space-y-4">
                <div className="bg-blue-50 p-3 rounded-xl flex items-center gap-2 border border-blue-100">
                  ...
                  <p className="text-[11px] text-blue-600 font-medium">데모 인증번호: 123456</p>
                </div>
