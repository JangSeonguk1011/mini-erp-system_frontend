...
const Sidebar = () => {
  ...
  /**
   * [수정] 권한별 동적 메뉴 설정
   * 권한에 따라 경로(path)의 앞부분을 /admin 또는 /manager로 동적 할당합니다.
   */
  ...
  const menuItems = [
    ...
    // 권한 부여는 관리소장(ADMIN)에게만 노출 (필요 시 팀장에게도 추가 가능)
    ...(isChief ? [{ 
    ...
    // [추가] 팀장도 본인의 연차를 관리할 수 있도록 메뉴 추가
    ...(!isChief ? [
    { 
      icon: <Send size={18} />, 
      label: '연차/특근 신청', 
    ...
    { 
      ...
      label: '연차/특근 결재', 
      path: `${basePath}/approvals` // [중요] 관리소장은 /admin/approvals, 팀장은 /manager/approvals로 연결
    },
  ];
  ...
  return (
    <div className="flex flex-col h-full p-6 bg-white shadow-sm border-r border-gray-100 overflow-y-auto">
      ...
      <div 
        ...
        onClick={() => navigate(`${basePath}/dashboard`)} // 로고 클릭 시 본인 대시보드로 이동
      >
      ...
      <nav className="flex-1 space-y-8">
        ...
        <div>
          ...
          <button
            onClick={() => navigate(`${basePath}/projects`)} // 프로젝트 경로도 권한별로 분리
